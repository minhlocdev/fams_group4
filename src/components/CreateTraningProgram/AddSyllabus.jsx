import { Autocomplete, Box, Button, Chip, Divider, IconButton, InputBase, ListItemIcon, Menu, MenuItem, Stack, TextField, Typography } from '@mui/material'
...
export default function AddSyllabus({ TraningProgramName, onClickBack }) {
    ...
    const handleSearch = (syl) => {
        setSelectedListSyllabus(prevSelectedListSyllabus => [...prevSelectedListSyllabus, syl]);
    ...
    return (
        <>
            <Box sx={{ width: 'calc(100% + 21px)', background: '#2D3748', paddingTop: 0.5, paddingBottom: 0.5, paddingLeft: 4, marginLeft: -2.5, marginTop: -0.3 }}>
                <Typography sx={{ color: 'white', pt: 1, pb: 1 }} variant="h6">Training program</Typography>
                <Stack direction="row" sx={{ alignItems: 'center', justifyContent: 'space-between' }}>
                    <Stack direction="row" spacing={2} sx={{ alignItems: 'center' }}>
                        <Typography sx={{ color: 'white', pt: 1, pb: 1 }} variant="h4">{TraningProgramName}</Typography>

                        {data.status === 'Active' ? (
                            <Chip sx={{ background: '#2D3748', color: 'white', borderColor: 'white' }} label={data.status} variant="outlined"
                            />
                        ) : data.status === 'Inactive' ? (
                            <Chip sx={{ background: '#B9B9B9', color: 'white', borderColor: 'white' }} label={data.status} variant="outlined"
                            />
                        ) : data.status === 'Draft' ? (
                            <Chip sx={{ background: '#285D9A', color: 'white', borderColor: 'white' }} label={data.status} variant="outlined"
                            />
                        ) : null}
                    </Stack>
            ...
            <Box sx={{ width: 'calc(100% + 21px)', paddingTop: 0.5, paddingBottom: 0.5, paddingLeft: 4, marginLeft: -2.5, marginTop: -0.3 }}>
            ...
            <hr></hr>
            <Box sx={{ width: 'calc(100% + 21px)', paddingTop: 0.5, paddingBottom: 0.5, paddingLeft: 4, marginLeft: -2.5, marginTop: -0.3 }}>
                <Typography sx={{ fontWeight: 'bold' }} variant="subtitle1" gutterBottom >Content</Typography>
                <Box sx={{ pt: 1, pb: 2 }}>
                    {SelectedListSyllabus?.map((Syllabus) => (
                        <SyllabusCard key={Syllabus.id} data={Syllabus} onDelete={handleDeleteSyllabus} />
                    ))}
                </Box>
                <Stack direction="row" spacing={2} sx={{ height: '32px', alignItems: 'center' }}>
                    <Typography sx={{ fontWeight: 'bold' }} variant="subtitle2" gutterBottom >Select syllabus</Typography>
                    <SearchSyllabus program={program} loading={loading} handleSearch={handleSearch} />
                </Stack>
            </Box>
            ...
            <Stack direction="row" sx={{ width: 'calc(100% + 21px)', paddingTop: 0.5, paddingBottom: 0.5, paddingLeft: 4, marginLeft: -2.5, marginTop: 5, justifyContent: 'space-between' }}>
                <Button variant="contained" sx={{ maxHeight: 27, background: '#2D3748' }} onClick={onClickBack}>Back</Button>
                <Stack direction="row" spacing={2} sx={{ justifyContent: 'flex-end', marginRight: 5 }}>
                    ...
                    <Button variant="contained" sx={{ maxHeight: 27, background: '#2D3748' }}>Save</Button>
                </Stack >
            </Stack >
        </>
    )
