...
export default function ImportTrainingProgram({ isOpen, handleClose }) {
  ...
  const csvData = [
    ["id", "syllabusName", "code", "createdOn", "createdBy", "duration", "outputStandard", "status"]
  ]
    return (
        <ModalContainer title={"Import Training Programs"} isOpen={isOpen} handleClose={handleClose} key={isOpen.toString()}>
            <Box sx={style}>
                        ...
                        <Box >
                            <Box>
                                ...
                                <FormGroup sx={{
                                    ...
                                    }} defaultChecked />} label="Program ID" />
                                    <FormControlLabel control={<Checkbox sx={{
                                        '&.Mui-checked': {
                                            color: '#2D3748',
                                        }
                ...
                <Stack direction="row" spacing={2} sx={{ justifyContent: 'flex-end', marginRight: 5 }}>
                    <Button variant="text" sx={{ color: '#E74A3B', textDecoration: 'underline', maxHeight: 27 }} onClick={handleClose}>Cancel</Button>
                    <Button variant="contained" sx={{ maxHeight: 27, background: '#2D3748' }} onClick={() => handleFile()}>Import</Button>
                </Stack >
            </Box>
        </ModalContainer >
    )
