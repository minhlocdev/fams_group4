  ...
  useEffect(() => {
    if (data) {
      setLearningObjectives(
        data.map(item => item.objectiveCode)
      );
    }
  }, [data, isSuccess]);
  
  const [selected, setSelected] = React.useState({
    outputStandards: checked.outputStandards ? checked.outputStandards.split(", "): [],
    dateRange: !(checked.createdDateBegin === "" || checked.createdDateEnd === "") && {
  ...
  return (
    <>
      <Grid gap={3}>
        ...
        <Grid item xs={6} m={2} >
          <Typography variant="h8" component="div">Class time frame</Typography>
          <DateRangePicker
